...
const generateAvailableSlots = (date: Date, startTime: Time, endTime: Time, slotDuration: number): CalendarSlot[] => {
...
const getOccupiedSlots = (events: CalendarEvent[], date: Date, slotDuration: number): CalendarSlot[] => {
  const occupiedSlots: CalendarSlot[] = []
...
const slotsOverlap = (slot1: CalendarSlot, slot2: CalendarSlot): boolean => {
