...
const is_slot_available_with_buffer_1 = require("./is-slot-available-with-buffer");
describe(`03 - ${is_slot_available_with_buffer_1.isSlotAvailableWithBuffer.name}`, () => {
    const availability = {
        include: [
            {
            ...
            {
                weekday: types_1.Weekday.saturday,
                range: [
                    { hours: 8, minutes: 0 },
                    { hours: 18, minutes: 0 },
                ],
            ...
            {
                weekday: types_1.Weekday.sunday,
                range: [
                    { hours: 8, minutes: 0 },
                    { hours: 18, minutes: 0 },
                ],
    ...
    it('should return true for an available slot without conflicting events and buffers', () => {
        ...
        const result = availableSlots.every(slot => (0, is_slot_available_with_buffer_1.isSlotAvailableWithBuffer)(availability, events, slot));
    ...
    it('should return false for a slot overlapping with an event and buffer', () => {
        ...
        const result = unavailableSlots.every(slot => (0, is_slot_available_with_buffer_1.isSlotAvailableWithBuffer)(availability, events, slot) === false);
        expect(result).toBe(true);
    });
});
